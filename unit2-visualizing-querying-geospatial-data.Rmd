...
This R notebook performs analysis for the Unit 2 project.
...
```{r}
# as.data.frame(bus.routes) %>% group_by(ROUTE_TYPE) %>% summarise()
```

```{r}
bike.routes <- st_read(here("Unit 1", "tahoe open data",
                            "Bike_Trails_Existing.geojson"))

bike.proposed <- st_read(here("Unit 1", "tahoe open data", 
                              "Bike_Trails_Proposed.geojson"))
```

```{r}
url <- here("Unit 2", "tahoe open data", "Tahoe_BlockGroup_2022_TDC_Values.geojson")
tdc.values <- st_read(dsn = url)
```

```{r}
url <- here("Unit 1", "tahoe open data", "2020_Regional_Transportation_Plan_Projects.geojson")
...
tm_shape(boundary, name = "TRPA Boundary") +
    ...
    tm_add_legend(type = "symbols",
                  ...
                  labels = c("TRPA Boundary", "Bus Stops", "Bus Routes",
                             "Priority Zones", "Bike Routes (Existing)",
                             "Bike Routes (Proposed)"), 
                  fill = c("darkblue", "red", "brown", "cyan",
                           "#4D7EAB", "purple"),
                  title = "Legend") +
...
```{r}
var.name <- "Vehicle_Available_0"
legend.title <- "% Household Without Vehicle"
var.name <- "Below_Poverty_Individual"
legend.title <- "% Population Below Poverty"
var.name <- "With_Disability"
legend.title <- "% Population With Disability"
# var.name <- "BIPOC"
# legend.title <- "% Population BIPOC Identity"
# var.name <- "Age_65_and_Over"
# legend.title <- "% Population Age 65+"

map.data <- tdc.values[, c("Total_Population", var.name)]
var.data <- as.data.frame(tdc.values)[, c(var.name)]

map.data$Percentage <- 
    round(var.data / map.data$Total_Population * 100, 0)
...
#colors <- c("lightgreen", "royalblue", "yellow", "coral", "red")
#colors <- c("#DBE1EA", "#b9ddf1", "#cc99bb", "#a6739b", "#7c4d79")
colors <- c("#D0DCEF", "#72BCDC", "royalblue", "#1F5591", "darkblue", "white")
...
north.zone <- subset(community.zones, 
                     ZoneName %in% c("Kings Beach", "Incline"))
...
tm_shape(north.zone) +
    ...
    tm_add_legend(labels = "Kings Beach & Incline", 
                  fill = "cyan",
                  position = c("bottom", "left")) +
    tm_credits(text = credits)
```
...
```{r}
south.zone <- subset(community.zones, 
                ZoneName %in% c("Tahoe Verde", "Sierra Tract", "Bijou"))
...
tm_shape(south.zone) +
    ...
    tm_shape(south.zone) +
    tm_polygons(col = "cyan", lwd = 2.5, fill = NULL) +
    tm_add_legend(labels = "Tahoe Verde, Sierra Tract, Bijou", 
                  fill = "cyan",
                  position = c("bottom", "left")) +
    tm_credits(text = credits)
```
...
map.data <- paste("Regional Transportation Plan 2025",
...
ggplot(chart.data, aes(y = Value, x = as.factor(Variable))) +
    ...
    labs(title = "Boxplot of High Transit Need Variables",
...
needs.data$Need_Score <- with(map.data, (Vehicle_Score + BIPOC_Score +
...
```{r}
# Bivariate map representing Access vs Need correlation
...
```{r}
colors <- c(
    "#d7dbdd", "#ebdef0", "#8e44ad",
    "#7dcea0", "#5d6d7e", "#512e5f",
    "#196f3d", "#0b5345", "#2e4053"
)

tm_shape(map.data) +
    tm_polygons(fill = "bi_class",
                fill.scale = tm_scale_categorical(values = colors),
                fill_alpha = 1,
                fill.legend = tm_legend(show = FALSE)) +
    tm_title(text = "Regional Transportation Plan 2025 - Transit Need & Access") +
    tm_credits(text = credits) +
    tm_scalebar()
```

```{r}
colors1 <- c("#D0DCEF", "#72BCDC", "royalblue", "#1F5591", "darkblue")
colors2 <- c("white", "yellow", "orange", "red", "darkred")

scale2 <- tm_scale_intervals(breaks = c(0, 1, 2, 3, 4, 5),
                            values = colors1,
                            style = "fixed")

scale1 <- tm_scale_intervals(breaks = c(0, 1, 6, 11, 16, 20, 25),
                            values = colors2,
                            style = "fixed")
...
chart.plot <- ggplot() +
    geom_sf(data = map.data, mapping = aes(fill = bi_class), 
...
ggdraw() +
    ...
    draw_plot(legend, 0.2, 0.1, 0.2, 0.2)
```
